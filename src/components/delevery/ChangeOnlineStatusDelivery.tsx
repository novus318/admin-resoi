...
const ChangeOnlineStatusDelivery = ({ order,
    refreshOrders,
}: any) => {
  ...
  const handleStatusChange = async (newStatus: 'pending' | 'confirmed' | 'in-progress' | 'completed' | 'cancelled') => {
    setLoading(true)
    try {
      const response = await axios.put(`${apiUrl}/api/online/update/online-order-status/${order.orderId}`, {
        status: newStatus,
      })
  ...
  const getAvailableStatuses = () => {
    if (status === 'pending') return ['confirmed','in-progress', 'completed', 'cancelled']
    if (status === 'confirmed') return ['in-progress', 'completed', 'cancelled']
    if (status === 'in-progress') return ['completed', 'cancelled']
    return []
  }
          ...
          <DropdownMenuContent>
            {getAvailableStatuses().map((availableStatus) => {
              ...
              return (
                <DropdownMenuItem
                  key={availableStatus}
                  onClick={() => handleStatusChange(availableStatus as 'pending' | 'confirmed' | 'in-progress' | 'completed' | 'cancelled')}
                >
...
export default ChangeOnlineStatusDelivery
