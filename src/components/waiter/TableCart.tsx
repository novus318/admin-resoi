...
const TableCart = ({ id,orderType }: any) => {
  ...
  const handleOrder = async () => {
    ...
    try {
      ...
      const orderDetails = {
        ...
        cartItems,
        tableId: id,
        userType: 'AdminUser', // Add the userType to the order
      ...
      const response = await axios.post(`${apiUrl}/api/tableOrder/create/table-order`, orderDetails);
        if (response.data.success) {
          toast({
            ...
            description:'please check your order section',
            variant: 'default'
          });
          dispatch(clearCart());
        }
  ...
  return (
    <Drawer>
      <DrawerTrigger asChild>
        <Button className='rounded-none fixed bottom-0 left-1/2 transform -translate-x-1/2 z-30 w-full'>
